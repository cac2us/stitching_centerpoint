...
for i in range(10000):
    color_list.append(np.random.randint(255, size=3))
...
def pred_interp(pred, scale):
    import pdb; pdb.set_trace()
    n_p = pred.shape[0]
    n_s = n_p // scale
    pred_1 = pred[:, 0][:n_s]
    pred_2 = pred[:, 1][:n_s]
    p_vals_1 = np.linspace(pred_1[0], pred_1[-1], 12)
    p_int = np.interp(p_vals_1, pred_1, pred_2)
...
def draw_box_in_bev(img,
    ...
    if boxes_tr is None:
        ...
        if ids is not None and len(ids):
            colors = np.repeat(color_list[ids], 4, 0)
        else:
        ...
        coors_range = np.array(coors_range)
        bev_corners = center_to_corner_box2d(boxes_tr[:, [0, 1]], boxes_tr[:, [3, 4]],
                                             boxes_tr[:, 6])
        bev_corners -= coors_range[:2]
        bev_corners *= np.array(
            img.shape[:2])[::-1] / (coors_range[3:5] - coors_range[:2])
        ...
        bev_lines = np.concatenate(
            [bev_corners[:, [0, 2, 3, 1]], bev_corners[:, [1, 3, 0, 2]]], axis=2)
        bev_lines = bev_lines.reshape(-1, 4)
        if ids_tr is not None and len(ids_tr):
            colors = np.repeat(color_list[ids_tr], 4, 0)
        else:
            colors = np.tile(np.array(color).reshape(1, 3), [bev_lines.shape[0], 1])
            colors = colors.astype(np.int32)
        img = cv2_draw_lines(img, bev_lines, colors, thickness)
    ...
    if preds is not None and len(preds):
        n_pred = preds[0].shape[0]
        for idx in range(preds.shape[0]):
            pred = preds[idx]
            #  box = boxes[idx]
            #  pred = np.vstack([box[:2], pred])
            #  pred += box[:2]
            pred -= coors_range[:2]
            pred *= np.array(
                img.shape[:2])[::-1] / (coors_range[3:5] - coors_range[:2])
            flag = ((pred > 1024).sum(1) + (pred < 0).sum(1)) == 0
            pred = pred[flag]
            #  for pc in pred:
                #  img = cv2.circle(img, (int(pc[0]), int(pc[1])), 1, (255, 0, 0), -1, cv2.LINE_AA)
            if False:
                z = np.polyfit(pred[:6, 0], pred[:6, 1], 2)
                pred[:6, 1] =  pred[:6, 0]**2*z[0] + pred[:6, 0]*z[1] + z[2]
            draw_points = (pred.astype(np.int32)[:])
            img = cv2.polylines(img, [draw_points], False, (255, 255, 0), 2)
...
def nuscene_vis(points, boxes=None, labels=None, preds=None, ids=None, boxes_tr=None, ids_tr=None):
    vis_voxel_size = [0.1, 0.1, 0.2]
    #  vis_point_range = [-51.2, -51.2, -5, 51.2, 51.2, 3]
    vis_point_range = [-43.9, -88.9, -3, 26.9, 22.7, 8]
    bev_map = point_to_vis_bev(points, vis_voxel_size, vis_point_range)
    if boxes is not None:
        bev_map = draw_box_in_bev(bev_map,
                                  vis_point_range,
                                  boxes, [0, 255, 0],
                                  2,
                                  labels,
