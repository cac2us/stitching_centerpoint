...
class Preprocess(object):
    ...
    def __call__(self, res, info):
        ...
        if res["type"] in ["WaymoDataset"]:
            ...
            points = res["lidar"]["combined"]
        elif res["type"] in ["StitchDataset"]:
            points = res["lidar"]["combined"]
        else:
...
class Voxelization(object):
    ...
    def __call__(self, res, info):
        ...
        x = res["lidar"]["points"]
        res['lidar']['points'] = x[~np.isnan(x).any(axis=1)]
        ...
        if res["mode"] == "train":
            ...
            for idx, task in enumerate(self.tasks):
                ...
                for k in range(num_objs):
                    ...
                    ### stitch to nuscenes format
                    w, l, h = gt_dict['gt_boxes'][idx][k][3], gt_dict['gt_boxes'][idx][k][4], \
                              gt_dict['gt_boxes'][idx][k][5]
                    # h, l, w = gt_dict['gt_boxes'][idx][k][3], gt_dict['gt_boxes'][idx][k][4], \
                    #           gt_dict['gt_boxes'][idx][k][5]                
