...
def get_sample_data(
    ...
    # for i in box_list:
    #     import pdb; pdb.set_trace()
    #     viz_box.append(box_list[i].center)
    #     viz_box.append(box_list[i].wlh)
    for box in box_list:
        viz_box.append(box.center)
        viz_box.append(box.wlh)

    np.save("box_org", viz_box)
...
def _fill_trainval_infos(nusc, train_scenes, val_scenes, test=False, nsweeps=1, filter_zero=True):
    from nuscenes.utils.geometry_utils import transform_matrix
    ...
    for sample in tqdm(nusc.sample):
        ...
        ref_lidar_path, ref_boxes, xxx = get_sample_data(nusc, ref_sd_token)
