...
def get_sample_data(nusc, sample_data_token,
    ...
    """ Function from NuScenes.get_sample_data
    To convert global coordinate -> ego local coordinate
    """
    def get_box_pred(pred):
        # convert tracking prediction to Box class
        trans, size, rot, oid, cls = pred
        boxes = []
        for idx in range(len(trans)):
            box = Box(trans[idx], size[idx], Quaternion(rot[idx]), name=cls[idx])
            boxes.append(box)
        return boxes

    def get_box_predl(pred, pred_list):
        # convert prediction point to Box class's center
        trans, size, rot, oid, cls = pred
        boxes = []
        for idx in range(len(pred_list)):
            for idx2 in range(12):
                box = Box(list(pred_list[idx][idx2]) + [0.], size[0], Quaternion(rot[0]), name=cls[0])
                boxes.append(box)
        return boxes
    ...
    if pred_list is not None:
        if len(pred_list):
            boxes = get_box_predl(pred, pred_list)
            boxl_list = []
            for box in boxes:
                if use_flat_vehicle_coordinates:
                    # Move box to ego vehicle coord system parallel to world z plane.
                ...
                boxl_list.append(box)
            preds_list = []
            for i in range(len(pred[0])):
                boxp_list = []
                for j in range(12):
                    boxp_list.append(boxl_list[i*12+j].center[:2])
                preds_list.append(np.array(boxp_list))
            preds_list = np.array(preds_list)
            return data_path, box_list, cam_intrinsic, preds_list

        return data_path, box_list, cam_intrinsic, pred_list
